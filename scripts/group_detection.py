...
def get_prediction(sequence, opt_pixel_thre=3, ratio=0.90, max_friends=30):
    '''
    Extract ego user's using group_detection
    '''   
    Detector = Group_Detection(data=sequence, dist_thre=opt_pixel_thre, ratio_thre=ratio)      
...
# =============================================================================            
            ...
            #print("ego_clusterLabel", ego_clusterLabel)
            #print("friendCandidate\n:", friendCandidate)
        ...
        for DB in DBscan_count:
            if (DB[1] / DB[2] > self.ratio_thre) and (DB[0] != egoUserId):
                friends.append(DB[0])
        return friends
